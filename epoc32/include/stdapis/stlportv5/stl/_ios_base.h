...
public:
  static void ios_base_S_was_synced_init()
  {
     get_ios_base_S_was_synced() = true;	
  }
protected:
  static inline bool& get_ios_base_S_was_synced();
#else
...
public:
  ...
  class _STLP_CLASS_DECLSPEC Init
  {
    public:
      ...
      _STLP_DECLSPEC ~Init();
# if defined(__SYMBIAN32__WSD__)
      static inline void ios_base_Init_S_count_init() 
      {
      	get_ios_base_Init_S_count() = 0;
      }
    private:
      static inline long& get_ios_base_Init_S_count();      
#else	
    private:  
